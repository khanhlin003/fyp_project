...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 bg-white/80 backdrop-blur-xl z-50 border-b border-zinc-200/60">
      <div className="w-full h-14 flex items-center px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-4 sm:gap-6 min-w-0">
          {/* Logo */}
          <Link href={isAuthenticated ? '/recommendations' : '/'} className="text-lg font-semibold text-zinc-900 tracking-tight flex items-center gap-2">
            <span className="bg-gradient-to-br from-indigo-500 to-violet-600 text-white w-7 h-7 rounded-md flex items-center justify-center text-xs">E</span>
            ETF Advisor
          </Link>
        ...
        <div className="ml-auto flex items-center gap-3">
          {isLoading ? (
            ...
            <>
              ...
              >
                <div className="w-7 h-7 bg-gradient-to-br from-indigo-500 to-violet-500 rounded-full flex items-center justify-center text-white text-xs font-semibold">
            ...
            <div className="flex items-center gap-2">
              <Link 
                href="/login" 
                className="text-zinc-500 hover:text-zinc-900 text-sm font-medium px-3 py-1.5 rounded-lg hover:bg-zinc-50 transition-colors"
              >
                Login
              </Link>
              <Link
                href="/signup"
                className="bg-indigo-500 text-white text-sm px-3 py-1.5 rounded-lg font-medium hover:bg-indigo-600 transition-colors shadow-sm shadow-indigo-500/25"
              >
                Sign Up
              </Link>
            </div>
